    ...
    <section className="mt-5 animate-fade-in" style={{ animationDelay: "80ms" }}>
      <div className="flex gap-3 px-5">
        {actions.map(({ icon: Icon, label, flex }) => (
          <button
            key={label}
            className={`group ${flex} flex items-center justify-center gap-2 rounded-xl border border-border bg-card px-3 py-3 shadow-card transition-all duration-200 hover:border-accent/50 active:scale-[0.97]`}
          >
